...
const Page: FaustPage<AuthorsPageQueryGetUsersBySearchQuery> = (props) => {
  ...
  // 클라이언트 측에서 role이 'MARKETER'인 사용자만 필터링
  let currentUsers = (initUsers || []).filter(user => user.role === 'MARKETER')
  ...
  if (getUsersBySearchResult.called) {
    currentUsers = [
      ...(initUsers || []),
      ...(getUsersBySearchResult.data?.users?.nodes || []).filter(user => user.role === 'MARKETER'),
    ]
